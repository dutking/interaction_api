...
class IterableScorableInteraction extends ScorableInteraction {
  ...
  getAmountOfUnits() {
    if (
      ...
      this.interactionData.amount_of_units > db[this.index].iterables.length
    ) {
...
class SurveyUnit extends InteractionUnit {
  ...
  getResults(e) {
    e.currentTarget.classList.add('disabled')
    Array.from(this.unitContainer.querySelectorAll('label')).forEach(function (
      l
    ) {
      l.classList.add('disabled')
    })
      ...
      if (w.includes('_')) {
        ...
        let modifiedLetters = letters.map(function (l) {
          if (l === '_') {
            ...
            for (
              let i = 0;
              i < that.optionsToInsert[wordIndex][spaceIndex].length;
              i++
            ) {
              let isLong =
