    ...
    get resultObj() {
        ...
        if (this.verb === "completed") {
            ...
            resultObj["result"] = {
                response:
                    "{" +
                    `"title":"${this.obj.title}","text":"${this.obj.text}","tag":"${this.obj.tag}"` +
                    "}",
            };
...
class App {
    ...
    static addRating() {
        if (
            document.querySelector('meta[content^="rating"]') &&
            document
    ...
    static addComment() {
        if (
            document.querySelector('meta[content^="comment"]') &&
            document
...
class Course {
    ...
    get score() {
        ...
        let val = Math.round((100 / this.interactions.length) * overallResult);
        return val;
    }
    ...
    set completed(v) {
        if (this.completed) {
            ...
            if (this.scoreImproved() === true) {
                console.log("Score improved!");
                console.log("Sending completion and success statements...");
                ...
                /* if (this.scoreChanged === true) {
                    this.stmtSent = true;
                } */
            } else {
                console.log(
                    "All statements have already been sent. Score didn't improve."
                );
            }
        } else {
            console.log("Course is still incompleted.");
        }
    }
}
...
class ScorableInteraction extends Interaction {
    constructor(index, renderHook, parent) {
        ...
        this._score = 0;
        this.minScore = Number(this.interactionData.min_score) || 0;
    }

    get score() {
        let val = Math.round(100 * this.interactionUnits[0].score);
        return val;
    }
...
class IterableScorableInteraction extends ScorableInteraction {
    ...
    getAmountOfUnits() {
        if (
            ...
            this.interactionData.amount_of_units >
                db[this.index].iterables.length
        ) {
    ...
    get completed() {
        if (this.interactionUnits.length < this.unitsToComplete) {
            ...
            this.interactionUnits.length >= this.unitsToComplete &&
            this.interactionUnits[this.unitsToComplete - 1].completed === true
        ) {
            return true;
        }
    }
    ...
    get score() {
        ...
        let val = Math.round((100 / this.amountOfUnits) * score);
        return val;
    }
...
class PointsDistributionInteraction extends Interaction {
    constructor(index, renderHook, parent) {
        ...
        this.autocomplete =
            this.interactionData.autocomplete === "true" ? true : false;
        this.init();
    }
...
class SurveyInteraction extends Interaction {
    ...
    createUnit(num) {
        let unit;
        unit = new SurveyUnit(num, this, "unit surveyUnit", db[this.index]);
        this.interactionUnits.push(unit);
    }
    ...
    init() {
        ...
        window.addEventListener("scroll", (event) => {
            if (App.isInViewport(this.renderHook)) {
                ...
                that.interactionUnits[0].score = 1;
                that.completed = true;
            }
        });
    }
...
class DictantInteraction extends ScorableInteraction {
    ...
    get score() {
        let val = Math.round(
            (100 / this.interactionUnits[0].fbs.length) *
                this.interactionUnits[0].score
        );
        return val;
    }

    createUnit(num) {
        let unit = new DictantUnit(
            num,
            this,
            "dictantUnit",
            db[this.index][num]
    ...
    render() {
        ...
        if (
            ...
            this.parent.parent.amountOfUnits -
                this.parent.parent.unitsToComplete >
                0
        ) {
        ...
        this.interactionUnits.forEach(function (unit, index) {
            unit.checkedStatus.forEach(function (i) {
                ...
                if (answer[0].aFeedback !== "") {
                    let aFb = unit.unitContainer.querySelector(
                        `.aFeedback[data-for=${i[0]}]`
                    );
                    if (i[1]) {
...
class VideoInteraction extends Interaction {
    ...
    set completed(v) {
        if (this.completed || this._completed) {
            this._completed = true;
            if (this.stmtSent === false) {
                console.log("Video completion stmts sent.");
                Xapi.sendStmt(v.stmtCompleted);
                Xapi.sendStmt(v.stmtPassed);
                Xapi.sendStmt(v.stmtExited);
                this.stmtSent = true;
                this.parent.completed = true;
            }
        }
    ...
    changeScreen() {
        if (this.counter <= this.numberOfScreens) {
            this.screenshot.setAttribute(
                "src",
                `./dist/assets/screenshots/${this.counter + 1}.png`
            );
            ...
            if (
                ...
                this.screpca.setAttribute(
                    "src",
                    `./dist/assets/yellow_screp.svg`
                );
                this.instructionElement.innerHTML = this.instruction[
                    this.counter
                ];
    ...
    clickCheck(e) {
        ...
        if (
            ...
            if (
                ...
                this.wrongText.innerHTML =
                    "Неверно!" +
                    '<div class="wrongbutton">Попробовать еще раз!</div>';
                this.wrongTextShow();
                this.helpDiv.style.display = "none";
            }
        }
...
class PointsDistributionUnit extends InteractionUnit {
  ...
  set done(v) {
    ...
    getResults(e) {
        e.currentTarget.classList.add("disabled");
        Array.from(this.unitContainer.querySelectorAll(".btnPrimary")).forEach(
            function (l) {
...
class PointsDistributionGroup {
    ...
    get values() {
        let values = Array.from(
            this.groupContainer.querySelectorAll(".btnPrimary")
        ).map(function (b) {
            return b.value;
        });
    ...
    updateCounter() {
        this.result = true;
        this.groupContainer.querySelector(
            "span.counter"
        ).innerHTML = this.pointsLeft;
        /* this.parent.done = true */
        if (this.pointsLeft === 0 && !this.values.includes("")) {
            this.groupContainer
                .querySelector(".groupHeader")
                .classList.add("done");
    ...
    render() {
        ...
        title.innerHTML = `${this.index + 1}/${
            this.parent.dbData.groups.length
        } <span class='groupName'>${this.group.name}</span>`;
        ...
        this.group.content.forEach(function (stmt, ind) {
            ...
            primaryBtn.addEventListener("click", function (e) {
                let allOptions = that.parent.unitContainer.querySelectorAll(
                    ".options"
                );
    ...
    isRepeated(v) {
        let that = this;
        let primaryBtns = Array.from(
            that.groupContainer.querySelectorAll(".btnPrimary")
        );
    ...
    setValue(event) {
        ...
        let primaryBtn = event.currentTarget.parentNode.parentNode.querySelector(
            ".btnPrimary"
        );
...
class SurveyUnit extends InteractionUnit {
    constructor(index, parent, cssClasses, dbData) {
        ...
        this.staticPositionedColumnsValues = dbData.staticPositionedColumnsValues
            ? dbData.staticPositionedColumnsValues
            : [];
    ...
    get itemsDone() {
        let itemsDone = this.unitContainer.querySelectorAll("input:checked")
            .length;
        return itemsDone;
    }
    ...
    render() {
        ...
        this.unitContainer.style.setProperty(
            "--columns",
            `${this.columnsNames.length}`
        );
    ...
    getResults(e) {
        e.currentTarget.classList.add("disabled");
        Array.from(this.unitContainer.querySelectorAll("label")).forEach(
            function (l) {
                l.classList.add("disabled");
            }
        );
        ...
        let inputs = Array.from(
            this.unitContainer.querySelectorAll("input:checked")
        );
        inputs.forEach(function (i, index) {
            ...
            that.surveyMetrics.forEach(function (m) {
                let allMetricQuestions = [
                    ...m.correspondingQuestions.normalScale,
                    ...m.correspondingQuestions.reverseScale,
                ];
                let questions;
                ...
                if (that.surveyMetrics.length === 1) {
                    ...
                    if (questions.includes(qNum)) {
                        metricId = m.id;
                        if (
                            m.correspondingQuestions.normalScale.includes(qNum)
                        ) {
                            scale = Array.from(that.columnsValues);
                        } else if (
                            m.correspondingQuestions.reverseScale.includes(qNum)
                        ) {
    ...
    showFb() {
      ...
      metricInfo.appendChild(metricName) */
            ...
            if (that.surveyMetricsRanges.length > 0) {
                ...
                rangeDescription.innerHTML =
                    that.surveyMetricsRanges[that.getRangeNum(m, resultNum)];
            ...
            if (that.fbs.length > 0) {
                that.fbs.forEach(function (fb) {
                    if (fb.type === "metricsResultsDescription") {
                        ...
                        fbContainer.classList.add("withDescription");
                        let metricResultDescription = document.createElement(
                            "p"
                        );
                        metricResultDescription.className =
        ...
        this.answers.forEach(function (ans, i) {
            ...
            if (ans.aFeedback !== "") {
                ...
                feedback.innerHTML = `${ans.aFeedback} ${
                    ans.aCorrect === false
                        ? '<p class="help">Выберите другой ответ.</p>'
                        : ""
                }`;
...
class TestUnit extends CmiInteractionUnit {
        ...
        if (this.response.length === 0) {
            ...
            this.checkedStatus.forEach(function (i) {
                ...
                if (that.parent.immediateFeedback === true) {
                    ...
                    if (answer[0].aFeedback !== "") {
                        let aFb = that.unitContainer.querySelector(
                            `.aFeedback[data-for=${i[0]}]`
                        );
                        if (i[1]) {
                            aFb.classList.remove("off");
    ...
    showFb() {
        ...
        fbHead.className = `fbHead leftBorderMarker ${
    ...
    <p>Проходной балл: ${this.parent.minScore}/${this.parent.amountOfUnits}.</p>`;
        ...
        if (this.parent.immediateFeedback === false) {
            ...
            showAllFbBtn.addEventListener(
                "click",
                that.parent.showAllFb.bind(that.parent)
            );
        }
        ...
        restartBtn.addEventListener(
            "click",
            that.parent.restart.bind(that.parent)
        );
    ...
    getData() {
        ...
        optionsData.forEach(function (optionData) {
            ...
            optionData.forEach(function (optionStr) {
                ...
                let isComa =
                    optionArr.includes("зпт") || optionArr.includes("зпт*");
                ...
                let modifiedLetters = letters.map(function (l) {
                    if (l === "_") {
                        ...
                        for (
                            let i = 0;
                            i <
                            that.optionsToInsert[wordIndex][spaceIndex].length;
                            i++
                        ) {
                            let isLong =
                                that.optionsToShow[wordIndex][spaceIndex][i]
                                    .length > 2;
                            let toShow =
                                that.optionsToShow[wordIndex][spaceIndex][i];
            ...
            spaces.forEach(function (e, spaceIndex) {
                if (
                    ...
                    let correctOptionToInsert =
                        that.optionsToInsert[wordIndex][spaceIndex][
                            correctOptionIndex
                        ];
                ...
                if (
                    that.optionsToReport[wordIndex][spaceIndex].includes(
                        "слитно"
                    ) ||
                    that.optionsToReport[wordIndex][spaceIndex].includes(
                        "ничего"
...
class LangExerciseUnit extends FillInDropDownUnit {
    ...
    onCompletedFired(wordIndex) {
        if (!this.userAnswer[wordIndex].includes("_")) {
            ...
            Xapi.sendStmt(
                new Statement(this.subUnits[wordIndex], "answered")
                    .finalStatement
            );
        }
...
function onPlayerStateChange(event) {
    if (event.data === 1) {
        ...
        App.course.interactions.forEach((i) => {
            if (i.interactionData.type === "video") {
                if (i.vidId === event.target.getVideoData().video_id) {
                    i.interactionUnits[0].completed = true;
                    i.completed = getVidData(event.target);
                }
            }
        });
