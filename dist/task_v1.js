...
function createTask(num) {
    ...
    taskWords.forEach(function(word, index) {
        ...
        /*                     let userExperiencedStmt = {
                        ...
                        object: {
                            objectType: 'Activity',
                            id: exerciseId + thisTask.taskId + '/word' + index + '/tip',
                            definition: {
                                name: {
                                    'ru-RU': 'Подсказка'
                                },
                                description: {
                                    'ru-RU': thisTask.tip[index]
                                }
                            }
                        },
                        context: {
                            contextActivities: {
                                parent: [
                                    {
                                        objectType: 'Activity',
                                        id: exerciseId + thisTask.taskId + '/word' + index,
                                        definition: {
                                            name: {
                                                'ru-RU': word
                                            },
                    ...
                    if (numberOfSpaces === 1 || (numberOfSpaces > 1 && i === numberOfSpaces - 1)) {
                        if (index < taskWords.length - 1) {
                            ...
                            btn.parentNode.parentNode.parentNode
                                .querySelectorAll('.answersContainer')
                                [index + 1].classList.remove('off')
                        }
                    } else if (numberOfSpaces > 1 && i < numberOfSpaces - 1) {
                        btn.parentNode.parentNode
                            .querySelectorAll('.choiceContainer')
                            [i + 1].classList.remove('off')
                    }
